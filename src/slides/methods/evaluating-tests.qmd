...
A <span class="highlight">joint test</span>

#### Definition

$$
t_k = \dfrac{\sqrt{n}\hat{\beta_k}}{\widehat{\avar}(\hat{\beta}_k)}
$$
...
Key question: how do you set the critical value $c_{\alpha}$ to ensure that the test has the correct size $\alpha$? Let's examine the rejection probabilities under $H_0$:
$$
\begin{aligned}
...
See [this xckd](https://xkcd.com/882/) on multiple comparisons
...
Very often: the nicest scenario is a simple linear model where everything in normal
...
In our case: think about what everyone uses
