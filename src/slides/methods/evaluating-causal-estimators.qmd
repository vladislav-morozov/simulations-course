...
subtitle: "Working with Causal Frameworks and Simulations for Counterexamples"
...
- Use process-based parallelism to accelerate computation
...
- Different simulation run structure: both estimators applied on the same dataset (not separately like before)
...
- Key package in using FE estimators well --- `pyfixest`
...
:::
Remember: before had a scenario per estimator


. . .

- In general: difference is small if data generation is cheap
- Trade-off: more cumbersome runners if want to apply array of estimators on each dataset (internal iteration over estimators)
