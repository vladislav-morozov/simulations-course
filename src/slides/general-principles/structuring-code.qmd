...
Things are all jumbled up and hardcoded  by`simulate_ols()` functions mixes everything
...
- DPS promises to give valid date if `sample` is called 
...
- We use classes to capture different concerns and specify interfaces
- Example
...
You can add any kind of of summaries (or make a different class that can process those results, make images, etc)
...
do not take this as an absolute truth, but more of a suggestion
