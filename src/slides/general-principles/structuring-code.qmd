...
def simulate_ols(n_sim=1000, n_obs=50, beta0=0, beta1=0.5, dgp_type="static"):
    ...
    for _ in range(n_sim):
        ...
        if dgp_type == "static":
            ...
            y = np.zeros(n_obs + 1)
            u = np.random.normal(size=n_obs + 1)
            y[0] = beta0 + u[0]
            for t in range(1, n_obs + 1):
                y[t] = beta0 + beta1 * y[t-1] + u[t]
            # Use the last n_obs elements of y and the first n_obs elements of y as x
...
```{python}
#| echo: true
ar_results = simulate_ols(dgp_type="ar1")
print(f"Bias (static DGP): {np.mean(ar_results)}")
```



#### Running AR(1) Simulation With High Persistence
 
```{python}
#| echo: true
ar_results = simulate_ols(beta1=0.95, dgp_type="ar1")
print(f"Bias (static DGP): {np.mean(ar_results)}")
```
