...
- *Could write docs*: that's good, but extra effort that a lot of people do not take until later
- *Could look at how `SimulationRunner` uses the estimator*: works when it's your fresh code. Otherwise harder, and can miss something
...
To use our protocols to communicate information both to readers and type checkers, just insert them in type hints and docstrings
...
### Aside: Alternative Approach {background="#43464B"}
...
- Specific estimators and DGPs inherit from 
can have  a more explicit inheritance relationship (e.g. every DGP inherits from abstract base class `BaseDGP`)
...
Protocols are really about implementation


ABCs


### Adding Lasso {background="#43464B"}

#### 

Another penalized SSR ("OLS-like") estimator is lasso, which solves

####

Do not want to implement it, a bit annoying

but `scikit-learn` has an implementation

problem is it's not compatible with our `EstimatorProtocol`
