...
Wrapper class that aligns `Lasso` with `EstimatorProtocol`:
```{python}
#| echo: true
from sklearn.linear_model import Lasso as SklearnLasso 
...
Today — calls a few scenarios (DGP + estimator + sample size) and prints bias summaries to the console 

(running many scenarios and handling outputs — next time)
...
if __name__ == "__main__":
    ...
    dgp = StaticNormalDGP(beta0=0.0, beta1=1.0)
    estimator = SimpleOLS()

    # Run simulation
    runner = SimulationRunner(dgp, estimator)
    runner.simulate(n_sim=1000, n_obs=100, seed=42)

    # Print results
    print(f"Bias for {dgp.__class__.__name__} + {estimator.__class__.__name__}: "
          f"{runner.summarize_bias():.4f}")

```

#### About `__name__` and `__main__`
