...
```{python}
static_dgp = StaticNormalDGP(beta0=0.0, beta1=0.5)
lasso = LassoWrapper(reg_param=0.001)

sim_lasso = SimulationRunner(static_dgp, lasso)
sim_lasso.simulate(n_sim=1000, n_obs=50, first_seed=1)
sim_lasso.summarize_bias() 
```

On average, lasso totally kills 
